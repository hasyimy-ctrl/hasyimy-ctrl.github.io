...
import { offsetsFor } from "./offsets.js?v=3";

export async function run(options) {
    const { onLog = console.log, onStatus = () => {} } = options;

    globalThis.mark = function(tag, detail) {
        onLog(tag, detail);
    };
    globalThis.state = function(msg, cls) {
        onStatus(msg, cls);
    };

    globalThis.outEl = document.getElementById('console');
    globalThis.stateEl = document.getElementById('statusText');
...
const SYS = { read: 3, write: 4, close: 6, getpid: 20, setuid: 0x17,
              getuid: 0x18, geteuid: 0x19, dup: 0x29, sendmsg: 0x1c, recvmsg: 0x1b,
...
async function runOriginal () {
    ...
    try {
        ...
        const kpatchName = off && off.kpatch ? "src/kpatch/" + off.kpatch
            : key ? "src/kpatch/" + key.replace(".", "") + ".bin" : null;
        ...
        try {
            const r = await fetch("src/payload.bin");
        ...
        if (uid === 0 || euid === 0) {
            mark("ALREADY-JAILBROKEN", "uid=0; exploit stopped");
